...
| 选择器 | 描述 |
|:-------|:---|
| ```" "```（单个空格）| 用单个空格将两个选择器隔开，表示选中 ```" "``` 前面元素的后代元素 |
...
| 选择器 | 描述 |
|:-------|:---|
| ```>```（大于号）| 用 ```>``` 连接两个或多个选择器，选择 ```>``` 前面元素的直接子元素 | 
...
| 选择器 | 描述 |
|:-------|:---|
|  ```+```（加号）| 用 ```+``` 连接两个选择器，选择 ```+``` 前面元素的相邻兄弟元素（用 ```+``` 连接的两个元素的父元素相同） |  
...
| 选择器 | 描述 |
|:-------|:---|
|  ```~```（波浪号）| 用 ```~``` 连接两个选择器，选择前面元素的所有兄弟元素（用 ```~``` 连接的两个元素的父元素相同） |  
...
| 选择器 | 描述 |
|:-------|:---|
|  ```xxx,xxx```（逗号）| 用 ```,``` 将两个或者多个选择器隔开，表示选择多个匹配的元素 |  
