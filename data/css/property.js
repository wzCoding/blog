...
export const Propertys = {
    ...
    O: [
        ...
        { code: 'orphans', desc: '设置块容器中必须显示在页面、区域或列底部的最小行数', lang: 'en-US' },
        { code: ':out-of-range', desc: 'CSS 伪类，表示一个 `<input>` 元素，其当前值处于属性 min 和 max 限定的范围外' },
        { code: 'outline', desc: 'CSS 简写属性， 是 `outline-style`, `outline-width` 和 `outline-color`的简写，设置轮廓属性，元素轮廓是绘制于元素周围的一条线，位于 `border` 的外围' },
        { code: 'outline-color', desc: '用于设置一个元素轮廓的颜色' },
        { code: 'outline-offset', desc: '用于设置 `outline` 与一个元素边缘或边框之间的间隙' },
        { code: 'outline-style', desc: '用于设置一个元素轮廓的样式' },
        { code: 'outline-width', desc: '用于设置一个元素的轮廓的厚度' },
        { code: 'overflow', desc: 'CSS 简写属性，是 `overflow-x`、`overflow-y`的简写，设置了元素溢出时所需的行为——即当元素的内容太大而无法适应它的块级格式化上下文时' },
        { code: 'overflow-anchor', desc: '提供一种退出浏览器滚动锚定行为的方法，该行为会调整滚动位置以最大程度地减少内容偏移' },
        { code: 'overflow-block', desc: '设置了当内容溢出盒的块首和块末侧时所显示的内容。可以不显示，或者显示滚动条或溢出内容' },
        { code: 'overflow-clip-margin', desc: '决定了带有 overflow:clip 的元素在其边界之外多远可以在被裁剪之前绘制', lang: 'en-US' },
        { code: 'overflow-inline', desc: '设置了当内容溢出盒的行首和行末侧时所显示的内容。可以不显示，或者显示滚动条或溢出内容' },
        { code: 'overflow-wrap', desc: '应用于行级元素，用来设置浏览器是否应该在一个本来不能断开的字符串中插入换行符，以防止文本溢出其行向盒' },
        { code: 'verflow-x', desc: '设置了当内容溢出块级元素的左右两侧时所显示的内容。可以不显示，或者显示滚动条或溢出内容' },
        { code: 'verflow-y', desc: '置了当内容溢出块级元素的上下两侧时所显示的内容。可以不显示，或者显示滚动条或溢出内容' },
        { code: 'overscroll-behavior', desc: 'CSS 简写属性，是 `overscroll-behavior-x`、`overscroll-behavior-y`的简写，控制浏览器过度滚动时的表现——也就是滚动到边界' },
        { code: 'overscroll-behavior-block', desc: '设置浏览器在到达滚动区域的块方向边界时的行为', lang: 'en-US' },
        { code: 'overscroll-behavior-inline', desc: '设置浏览器在到达滚动区域的内联方向边界时的行为', lang: 'en-US' },
        { code: 'overscroll-behavior-x', desc: '用来控制当滚动到区域的水平边界时的浏览器行为' },
        { code: 'overscroll-behavior-y', desc: '用来控制当滚动到区域的垂直边界时的浏览器行为' },
    ],
    P: [
        { code: 'Pseudo-classes', desc: '伪类，伪类是添加到选择器的关键字，用于指定所选元素的特殊状态' },
        { code: 'Pseudo-elements', desc: '伪元素，伪元素是一个附加至选择器末的关键词，允许你对被选择元素的特定部分修改样式' },
        { code: 'pc', desc: 'CSS 数据类型，用于表示距离尺寸', linkParam: 'length#pc' },
        { code: 'pt', desc: 'CSS 数据类型，用于表示距离尺寸', linkParam: 'length#pt' },
        { code: 'px', desc: 'CSS 数据类型，用于表示距离尺寸', linkParam: 'length#px' },
        { code: 'pad', desc: '此属性和自定义记数样式一起使用时，可以用来限定记数样式的最小长度。如果某个记数样式小于 `pad` 所指定的长度，那么会用 `pad` 指定的填充字符串来填充至指定长度。如果比 `pad` 所指定的长度长，则会正常显示', linkParam: '@counter-style/pad' },
        { code: 'padding', desc: 'CSS 简写属性，是 `padding-bottom`、`padding-left`、`padding-right`、`padding-top`的简写，控制元素所有四条边的内边距区域' },
        { code: 'padding-block', desc: '定义了元素的逻辑块首和块末内边距，并根据元素的书写模式、行内方向和文本朝向对应至实体内边距' },
        { code: 'calc()', desc: 'CSS 函数，允许在声明 CSS 属性值时执行一些计算' },
        { code: 'calc()', desc: 'CSS 函数，允许在声明 CSS 属性值时执行一些计算' },
        { code: 'calc()', desc: 'CSS 函数，允许在声明 CSS 属性值时执行一些计算' },
        { code: 'calc()', desc: 'CSS 函数，允许在声明 CSS 属性值时执行一些计算' },
